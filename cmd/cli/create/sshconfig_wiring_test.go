...
// TestRunSingleNodeProvision_StrictPolicyReachesDialer drives the production
// runSingleNodeProvision -> provisioner.New call site and asserts that a
// strict knownHostsPolicy set on the environment's auth.sshConfig actually
// reaches the Dialer. runSingleNodeProvision is unexported, but it is the
// exact function NewCommand's create Action reaches (via runProvision) once
// provisioning starts, so this test exercises it directly rather than
// scaffolding the full CLI command.
//
...
// option is dropped from runSingleNodeProvision. Mirrors
...
func TestRunSingleNodeProvision_StrictPolicyReachesDialer(t *testing.T) {
	dir := t.TempDir()
	...
	err := runSingleNodeProvision(logger.NewLogger(), opts)
	...
	assert.Contains(t, err.Error(), "strict",
		"the strict host-key policy must reach the Dialer through the production "+
			"runSingleNodeProvision -> provisioner.New call site")
}
